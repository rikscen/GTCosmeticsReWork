...
namespace CoreFramework
{
    public class DBTable
    {
        ...
        public String GenerateUpdateTabulation(Dictionary<String, String> updateList, Dictionary<String, String> primary_values, String compare_field, String compare_value)
        {
            String result = "";
            char[] delim = { ' ' };
            String compare_sql = "";
            ...
            for (int i = 0; i < columns.Count; i++)
            {
                ...
                if ("prodindex" != cleaned.ToLower())
                    result += String.Format(" {0} =  [{0}] + " + delimiter + "{1}" + delimiter + " " + ((i == columns.Count - 1) ? "" : ","), cleaned, value);
            }
            result += GenerateFilter(primary_values);
            result = compare_sql + result;
            return result;
        ...
        public String GenerateCreateUpdateTabulation(Dictionary<String, String> insertList, Dictionary<String, String> updateList, Dictionary<String, String> primary_values)
        {
            ...
            result += String.Format(@"IF EXISTS (SELECT TOP 1 * FROM {0} {1})
                            ...
                            BEGIN
	                            {3};
                            END;", table.Split(delim)[0], GenerateFilter(primary_values), GenerateUpdateTabulation(updateList, primary_values, null, null), GenerateInsert(insertList));
